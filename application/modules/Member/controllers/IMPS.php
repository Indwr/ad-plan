...
class IMPS extends CI_Controller {
    ...
    public function __construct() {
       ...
       $this->api_token = "934528020232998";
    }
    ...
    public function moneyTransfer($id=''){
        if (is_logged_in()) {
            ...
            $limitWithdraw2 = $this->User_model->get_single_record('tbl_money_transfer', ' user_id = "' . $this->session->userdata['user_id'] . '" AND (status = "SUCCESS" OR status = "ACCEPTED")', 'count(id) as limitWithdraw');
            if($limitWithdraw2['limitWithdraw'] == 0){
                $response['with_limit'] = 400;
            }else{
                $response['with_limit'] = 400;
            }
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                    ...
                    if ($this->form_validation->run() != FALSE) {
                        ...
                        if(!empty($checkBeneficary)){
                            // if($data['otp'] == $_SESSION['verification_otp'] && !empty($_SESSION['verification_otp'])){
                                ...
                                $directs = $this->User_model->get_single_record('tbl_users', ' sponser_id = "' . $this->session->userdata['user_id'] . '" AND paid_status > 0 AND date(topup_date) >= "2021-08-09"', 'count(id) as ids');
                                ...
                                $limitWithdraw = $this->User_model->get_single_record('tbl_money_transfer', ' user_id = "' . $this->session->userdata['user_id'] . '" AND (status = "SUCCESS" OR status = "ACCEPTED")', 'count(id) as limitWithdraw');
                                if($limitWithdraw['limitWithdraw'] == 0){
                                    $with = 400;
                                }else{
                                    $with = 400;
                                }
                                if(empty($today_money)){
                                    if ($withdraw_amount >= $with && $withdraw_amount <= 5000) {
                                        if($directs['ids'] >= 1){
                                        if ($withdraw_amount % $with == 0) {
                                            if ($balance['balance'] >= $withdraw_amount) {
                                                if ($user['master_key'] == $master_key AND $checkBeneficary['account_ifsc'] != '6244482379_IDIB000R072') {
                                                    // if($kyc_status['kyc_status'] == 2){
                                                        ...
                                                        $paramList = array('apikey' => $this->api_token,'mobileno' => $checkBeneficary['beneficiary_mobile'], 'beneficiary_account_no' => $checkBeneficary['beneficiary_account_no'], 'beneficiary_ifsc' => $checkBeneficary['beneficiary_ifsc'], 'amount' => $transfer_amount, 'orderid' => $myorderid, 'purpose' => 'BONUS', 'remarks' => 'ADLIFE', 'callbackurl' => $callBackUrl);
                                                        //print_r($paramList);
                                                        ...
                                                        if(!empty($jsondata)){ 
                                                            ...
                                                            if($jsondata['status'] == 'ACCEPTED' || $jsondata['status'] == 'SUCCESS'){
                                                                ...
                                                                $message = 'Dear '.$user['name'].' You have Successfully Received of amount Rs.'.$withdraw_amount.' For More Details Please Visit: https://adlife.live/';
                                            ...
                                            $this->session->set_flashdata('error', 'Withdraw Amount is multiple of '.$with.'');
                                        }
                                        } else {
                                            $this->session->set_flashdata('error', 'For Withdraw 1 direct required!');
                                        }
                                    } else {
                                        $this->session->set_flashdata('error', 'Minimum Withdrawal Amount is Rs '.$with.'');
                                    }
                                }else{
                ...
                $response['withdraw'] = '400, Multiple Rs. 400 & Maximum Unlimited';
