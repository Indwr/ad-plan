...
class Cron extends CI_Controller {
    ...
    public function createDownline($value='')
    {
       $users = $this->User_model->get_records('tbl_users', array('user_id !=' => 'admin'), 'id,user_id');
       foreach ($users as $key => $user) {
        //pr($user);
          $this->add_counts($user['user_id'], $user['user_id'], '1');
          $this->add_sponser_counts($user['user_id'], $user['user_id'], '1');
       }
    }
