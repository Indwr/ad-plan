...
//                    $real_date = '08/08/2019';
    ...
    function uploadAds() {
        if (is_admin()) {
            $response['header'] = 'Upload Ads';
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                $totalAds = $this->Main_model->get_single_record('tbl_ads', array('status' => 0), 'count(id) as totalAds');
                if( $totalAds['totalAds'] < 10){
                    $data = $this->security->xss_clean($this->input->post());
                    ...
                    //if ($data['type'] == 'image') {
                        if (!empty($_FILES['media']['name'])) {
                            ...
                            if (!$this->upload->do_upload('media')) {
                                $error = array('error' => $this->upload->display_errors());
                                $response = $this->session->set_flashdata('error', $this->upload->display_errors());
                                $this->load->view('uploadAds.php', $response);
                            } else {
                                ...
                                //die('here');
                                $fileData = array('upload_data' => $this->upload->data());
                                $userData['file'] = $fileData['upload_data']['file_name'];
                                $userData['type'] = $data['type'];
                                $userData['time'] = $data['time'];
                                $userData['caption'] = $this->input->post('caption');
                                $updres = $this->Main_model->add('tbl_ads', $userData);
                                if ($updres == true) {
                                    $response = array('error' => 'Ad Uploaded Successfully');
                                    $this->session->set_flashdata('error', 'Popup Uploaded Successfully');
                                } else {
                                    $response = array('error' => 'There is an error while uploading Ad, Please try Again ..');
                                    $this->session->set_flashdata('error', 'There is an error while uploading ad, Please try Again ..');
                                }
                            }
                        } else {
                            $response = array('error' => 'There is an error while uploading ad, Please try Again ..');
                    ...
                    // }
                        ...
                        $this->session->set_flashdata('error', '10 Ads Already Uploaded!');
                    }
                    redirect('Jarvis/Management/uploadAds');
            } else {
                // $response = $this->session->set_flashdata('error', 'Validation Failed');
                $this->load->view('uploadAds.php', $response);
            }
    ...
    public function uploadAdsHistory() {
        if (is_admin()) {
            $response['news'] = $this->Main_model->get_records('tbl_ads', array('status' => 0), '*');
            $this->load->view('uploadAdsHistory', $response);
        } else {
            redirect('Jarvis/Management/login');
        }
    }
    ...
    public function blockAd($id=''){
        if (is_admin()) {
            $res = $this->Main_model->update('tbl_ads', array('id' => $id), array('status' => 1));
            $this->session->set_flashdata('message', 'Ad Remove Successfully!');
            redirect('Jarvis/Management/uploadAdsHistory');
        } else {
            redirect('Jarvis/Management/login');
        }
    }
